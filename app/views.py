...
def index():
	...
	if request.form.get('edit') and current_user.is_authenticated():
		...
		if len(tname)>0 and len(tmark)>0:
			k.execute('UPDATE posts SET name=(?),mark=(?) WHERE id=(?)',[tname,tmark,id])
			k.commit()
		else:
		...
		if mark!=None and name!='':
                	k.execute('INSERT INTO posts(username,name,mark)VALUES(?,?,?)',[username,name,mark])
                	k.commit()
	try:
                k=k.execute('select id,name,mark from posts')
        except:
                k=None
